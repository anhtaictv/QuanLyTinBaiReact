...
- **Interview transcription** (`/transcribe`): live mic dictation (Web Speech API) or upload a recording to transcribe via PhoWhisper on the local AI host, auto-chunked into 25s segments to work around the Whisper pipeline's 30s limit
- **AI voice** (`/ai-voice`): clone a voice from a single sample audio file via **VoiceStudio** running on the local AI host, then use that voice to read back any text (up to 4000 chars/request) — a standalone module, generated audio is never persisted
- Every request goes through a shared **AI Gateway** (`aiGatewayClient.js`, OpenAI-compatible `/v1/chat/completions` + `/v1/embeddings` contract) that tries a self-hosted Ollama instance first and falls back to a cloud model when it's unreachable — the app itself never talks to Ollama or a cloud provider directly; every AI route returns a clean `503` when the gateway is unreachable instead of failing the request. Transcription and AI voice are gateway-specific extensions (`/transcribe`, `/voices`, `/speech`) that run PhoWhisper/VoiceStudio on the same host, with no cloud fallback for voice cloning (Claude/OpenAI don't support it)
...
frontend/                    # React SPA
  src/
    ...
    views/                      # Login, Dashboard, NewsList, NewsForm, Chat, NewsDigest, AiAssistant, AIKnowledge, InterviewTranscribe, AiVoiceStudio, ...
  vite.config.js
```
...
See [Releases](../../releases) for the full history of changes. Recent highlights: an **AI Voice** module (`/ai-voice`) — clones a voice via VoiceStudio running on the local AI host, then reads back any text in that voice, through two new AI Gateway extensions (`/voices`, `/speech`); a **Task Calendar / Assignment** module (`/tasks`, `/tasks/assign`) with an upcoming-deadlines panel on the Dashboard; **Interview Transcription** moved from a slide-out panel into its own `/transcribe` page, supporting both live mic dictation and uploading a recording (auto-chunked into 25s segments to work around the local Whisper server's 30s limit); the AI assistant upgraded from "assist" to "oversight" — fact-check and consistency-checking against the approved-articles/style-guide corpus via RAG, a visual diff between a contributor's original submission and the edited version (via Google Drive Revisions), per-article pinned AI chat, an installable PWA, and a SHA-256 hash-chain audit trail guarding approved articles against silent edits; a revamped Content Generator (9 templates); and quality fixes to the editorial assistant (proofreading and headline suggestions now use tuned temperature and stricter prompts to stay faithful to the source text instead of drifting or going generic). Earlier: the AI assistant migrated from calling Ollama directly to a shared **AI Gateway**, so the app now degrades to a cloud fallback instead of losing AI entirely when the local model host is offline; an automated local news digest pulling and deduplicating regional coverage from RSS and Google News; and a full move off Create React App onto Vite (~66% smaller initial bundle via route-level code-splitting), an SQL Server driver upgrade across six majors, and the operational layer described above (health check, log rotation, automated backups, Telegram alerts, CI/CD, and the project's automated test suite).
