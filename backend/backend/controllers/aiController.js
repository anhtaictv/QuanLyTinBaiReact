...
exports.health = async (req, res) => {
    try {
        const response = await fetch(`${aiGateway.BASE_URL}/health`, {
        ...
        res.json({
            ...
            phoWhisper: data.ollama || data.fallbackProvider !== 'none' ? 'online' : 'offline',
            voiceStudio: data.ollama || data.fallbackProvider !== 'none' ? 'online' : 'offline'
        });
    } catch (err) {
        res.json({
            aiGateway: 'offline',
