...
- **Rã băng phỏng vấn** (`/transcribe`): đọc trực tiếp bằng micro (Web Speech API) hoặc upload file ghi âm để chuyển thành văn bản qua PhoWhisper trên máy chạy AI cục bộ, tự cắt đoạn 25 giây để vượt giới hạn 30s của pipeline Whisper
- **Giọng đọc AI** (`/ai-voice`): nhân bản giọng nói từ 1 file audio mẫu (voice cloning) qua **VoiceStudio** chạy trên máy AI cục bộ, rồi dùng giọng đó để đọc bất kỳ đoạn văn bản nào (tối đa 4000 ký tự/lượt) — module độc lập, không lưu lại audio đã sinh
- Mọi request đều đi qua **AI Gateway** dùng chung (`aiGatewayClient.js`, theo đúng hợp đồng OpenAI `/v1/chat/completions` + `/v1/embeddings`) — gateway tự thử máy Ollama tự triển khai trước, chuyển sang model đám mây khi không tới được; app không bao giờ gọi thẳng Ollama hay nhà cung cấp đám mây; mọi route AI trả về `503` gọn gàng khi gateway không kết nối được, thay vì làm lỗi cả request. Riêng rã băng và giọng đọc AI là extension riêng của gateway (`/transcribe`, `/voices`, `/speech`) chạy PhoWhisper/VoiceStudio trên cùng máy, không có dự phòng đám mây cho voice cloning (Claude/OpenAI không hỗ trợ)
...
frontend/                     # React SPA
  src/
    ...
    views/                       # Login, Dashboard, NewsList, NewsForm, Chat, NewsDigest, AIKnowledge, InterviewTranscribe, AiVoiceStudio, ...
  vite.config.js
```
...
Xem [Releases](../../releases) để biết đầy đủ lịch sử thay đổi. Điểm nổi bật gần đây: module **Giọng đọc AI** (`/ai-voice`) — nhân bản giọng nói (voice cloning) qua VoiceStudio chạy trên máy AI cục bộ rồi đọc bất kỳ văn bản nào bằng giọng đó, đi qua 3 extension mới của AI Gateway (`/voices`, `/speech`); module **Lịch công việc / Giao việc** (`/tasks`, `/tasks/assign`) kèm panel việc sắp tới hạn trên Dashboard; tính năng **Rã băng phỏng vấn** tách từ panel trượt thành trang riêng `/transcribe`, hỗ trợ cả đọc trực tiếp bằng mic lẫn upload file ghi âm (tự cắt đoạn 25 giây để vượt giới hạn 30s của server Whisper nội bộ); trợ lý AI nâng cấp từ "hỗ trợ" lên "giám sát" — fact-check và kiểm tra nhất quán đối chiếu với corpus bài đã duyệt/cẩm nang tòa soạn qua RAG, visual diff giữa bản gốc CTV gửi và bản đang sửa (qua Google Drive Revisions), chat AI ghim theo từng bài viết, PWA installable, và audit trail hash-chain SHA-256 chống sửa ngầm bài đã duyệt; Content Generator làm mới với 9 template; và các bản vá cải thiện chất lượng trợ lý biên tập (sửa lỗi chính tả và gợi ý tiêu đề nay dùng temperature đã tinh chỉnh cùng prompt chặt hơn để bám sát nội dung gốc thay vì lệch ý hoặc chung chung). Trước đó: trợ lý AI (sửa lỗi/chuẩn hóa văn phong, gợi ý tiêu đề/tóm tắt, phân loại & gắn tag tự động, rà soát thông tin nhạy cảm, tra cứu quy định qua RAG, và thêm màn hình hỏi đáp tự do) chuyển từ gọi thẳng Ollama sang dùng chung **AI Gateway**, nhờ đó app tự chuyển sang model đám mây thay vì mất hẳn AI khi máy chạy model cục bộ offline; tính năng tổng hợp tin địa phương tự động, chống trùng, lấy từ RSS báo chí và Google Tin tức; và chuyển hẳn frontend từ Create React App sang Vite (bundle ban đầu nhỏ hơn ~66% nhờ code-split theo route), nâng driver SQL Server qua 6 major version, cùng toàn bộ lớp vận hành nói trên (health check, log xoay vòng, backup tự động, cảnh báo Telegram, CI/CD, và bộ test tự động của project).
