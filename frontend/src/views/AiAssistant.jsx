...
import { IconSend, IconLoader, IconTrash, IconAlertCircle, IconRefresh, IconSparkles } from '../components/icons';
...
const AiAssistant = () => {
  ...
  useEffect(() => {
    let alive = true;
    getAiHealth()
      .then(res => { if (alive) setConnected(!!res.data?.connected); })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 190px)', minHeight: 420 }}>
      ...
      <header style={{
        ...
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 21, fontWeight: 700, color: 'var(--text)', margin: 0, display: 'flex', alignItems: 'center', gap: 9 }}>
            <IconSparkles size={20} style={{ color: 'var(--accent)' }} />Trợ lý AI
          </h1>
